...
//MARK:Utilitarian family
        case .utilitarianLarge:
            template = CLKComplicationTemplateUtilitarianLargeFlat(textProvider: pizzaStage(stage: stage), imageProvider: image(stage: stage))
        case .utilitarianSmall:
            template = CLKComplicationTemplateUtilitarianSmallRingImage(imageProvider: image(stage: stage), fillFraction: stages.fillFraction(stage:stage), ringStyle: .closed)
        case .utilitarianSmallFlat:
            template = CLKComplicationTemplateUtilitarianSmallFlat(textProvider: pizzaStage(stage:stage), imageProvider: image(stage: stage))
//MARK:Extra large
...
//MARK:Graphic Family
        case .graphicExtraLarge:
            if complication.identifier == "customComplication"{
                ...
                template = CLKComplicationTemplateGraphicExtraLargeCircularClosedGaugeView(gaugeProvider: ringGauge(stage: stage), label: Text(stages.emoji(stage)))
            }
       ...
       case .graphicBezel:
        let gc = CLKComplicationTemplateGraphicCircularOpenGaugeSimpleText(gaugeProvider: ringGauge(stage: stage), bottomTextProvider: estimatedTime(from: now), centerTextProvider: pizzaEmoji(stage: stage))
        template = CLKComplicationTemplateGraphicBezelCircularText(circularTemplate: gc, textProvider: pizzaStage(stage: stage))
        case .graphicCircular:
            template = CLKComplicationTemplateGraphicCircularOpenGaugeSimpleText(gaugeProvider: ringGauge(stage: stage), bottomTextProvider: estimatedTime(from: now), centerTextProvider: pizzaEmoji(stage: stage))
        case .graphicCorner:
            template = CLKComplicationTemplateGraphicCornerGaugeText(gaugeProvider: ringGauge(stage: stage), leadingTextProvider: estimatedTime(from: now), trailingTextProvider: nil, outerTextProvider: pizzaEmoji(stage: stage))
        default:
            template = nil
    ...
    func createOrderTimeline(for complication: CLKComplication)->[CLKComplicationTimelineEntry]?{
        ...
        for stage in 1..<stages.count{
            timelineEntries?.append(createTimeLineEntry(for: complication, date: now.addingTimeInterval(stages.times(stage)), stage: stage)!)
        }
        timelineEntries?.append(createTimeLineEntry(for: complication, date: now.addingTimeInterval(stages.totalTime), stage: 0)!)
